...
def log_training_loss(step: int, loss: float, csv_path: str):
    """
...
DATASET_CONFIG = {
    "mnist": {
        "in_channels": 1,
        "num_classes": 10,
        "default_csv": "../results/training_metrics_mnist_study_rank.csv",
...
def train(args, model, device, train_loader, optimizer, privacy_engine, epoch):
    ...
    # Global step counter (epoch-aware)
...
def run_experiments(args, device, train_loader, test_loader):
    ...
    for seed in sketched_seeds:
        ...
        for compression_rate in sketched_compression_rates:
            for nb_dims_pca in [1,2,5,10,20,50,100,200,500,1000,2000]:
                for remove_dom in [True]:
