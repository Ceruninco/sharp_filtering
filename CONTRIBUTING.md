...
```bash
isort -l 88 -o opacus_local --lines-after-imports 2 -m 3 --trailing-comma  .
```
from the repository root. Configuration for isort is located in .isort.cfg.
If using `isort` versions `<5.0.0` call
```bash
isort -l 88 -o opacus_local --lines-after-imports 2 -m 3 --trailing-comma --recursive
```
...
```bash
python -m pytest --doctest-modules -p conftest opacus_local
```
