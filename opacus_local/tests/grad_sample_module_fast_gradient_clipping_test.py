...
from opacus_local.grad_sample import GradSampleModule, GradSampleModuleFastGradientClipping
from opacus_local.optimizers import DPOptimizer, DPOptimizerFastGradientClipping
from opacus_local.utils.fast_gradient_clipping_utils import DPLossFastGradientClipping
from opacus_local.utils.per_sample_gradients_utils import clone_module
...
class GradSampleModuleFastGradientClippingTest(GradSampleModuleTest):
    ...
    def test_norm_calculation_fast_gradient_clipping(self, size, length, dim):
        """
        Tests if norm calculation is the same between standard (opacus_local) and fast gradient clipping"
        """
        self.length = length
        self.size = size
        self.dim = dim
    ...
    def test_gradient_calculation_fast_gradient_clipping(self, size, length, dim):
        """
        Tests if gradients are the same between standard (opacus_local) and fast gradient clipping"
        """
