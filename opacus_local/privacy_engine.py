...
import torch
from opacus_local.accountants import create_accountant
from opacus_local.accountants.utils import get_noise_multiplier
from opacus_local.data_loader import DPDataLoader, switch_generator
from opacus_local.distributed import DifferentiallyPrivateDistributedDataParallel as DPDDP
from opacus_local.grad_sample import (
    AbstractGradSampleModule,
    GradSampleModule,
    get_gsm_class,
    wrap_model,
)
from opacus_local.optimizers import DPOptimizer, get_optimizer_class
from opacus_local.schedulers import _GradClipScheduler, _NoiseScheduler
from opacus_local.utils.fast_gradient_clipping_utils import DPLossFastGradientClipping
from opacus_local.validators.module_validator import ModuleValidator
...
class PrivacyEngine:
    ...
    def __init__(self, *, accountant: str = "prv", secure_mode: bool = False):
        ...
        Args:
            accountant: Accounting mechanism. Currently supported:
                - rdp (:class:`~opacus_local.accountants.RDPAccountant`)
                - gdp (:class:`~opacus_local.accountants.GaussianAccountant`)
                - prv (:class`~opacus_local.accountants.PRVAccountant`)
            secure_mode: Set to ``True`` if cryptographically strong DP guarantee is
                ...
                See :meth:`~opacus_local.optimizers.optimizer._generate_noise` for details.
    ...
    def get_compatible_module(cls, module: nn.Module) -> nn.Module:
        ...
        Returns:
            ...
            close equivalents.
            See :class:`~opacus_local.validators.module_validator.ModuleValidator` for
            more details
        """
    ...
    def make_private(
        ...
        **kwargs,
        ...
        Args:
            ...
            clipping: Per sample gradient clipping mechanism ("flat" or "per_layer" or "adaptive" or "dome").
            ...
            grad_sample_mode: mode for computing per sample gradients. Determines the
                ...
                :class:`~opacus_local.grad_sample.gsm_base.AbstractGradSampleModule` for more
                details
    ...
    def make_private_with_epsilon(
        ...
        **kwargs,
        ...
        """
        Version of :meth:`~opacus_local.privacy_engine.PrivacyEngine.make_private`,
        that calculates privacy parameters based on a given privacy budget.

        For the full documentation see
        :meth:`~opacus_local.privacy_engine.PrivacyEngine.make_private`
